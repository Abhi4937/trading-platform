...
import type {
  M7AggregateResponse, M7BestComboRow, M7CostBreakdown, M7ExitRule, M7Filters,
  M7IvBandSummaryRow, M7Meta, M7PathResponse, M7Summary, M7TradesResponse,
} from '../types/m7';
...
export function fetchM7Summary(filters: M7Filters = {}, exit_rule?: M7ExitRule): Promise<M7Summary> {
  return jsonFetch<M7Summary>(`${BASE}/summary${buildQuery(filters as Record<string, unknown>, exit_rule)}`);
}
...
export function fetchM7IvBandSummary(opts: { metric?: string } = {},
                                      exit_rule?: M7ExitRule): Promise<{ rows: M7IvBandSummaryRow[]; metric: string }> {
  return jsonFetch(`${BASE}/iv_band_summary${buildQuery(opts as Record<string, unknown>, exit_rule)}`);
}
