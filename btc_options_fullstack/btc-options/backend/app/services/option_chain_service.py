...
async def get_expiries() -> ExpiryListResponse:
    ...
    prods_task = asyncio.create_task(_get_products_cached())
...
async def get_option_chain(expiry: date) -> OptionChainResponse:
    # Products cached (stable), spot and tickers always live
    client = get_delta_client()
    spot_task    = asyncio.create_task(client.get_spot_price())
    prods_task   = asyncio.create_task(_get_products_cached())
    ...
    # Delta Exchange settles at 5:30 PM IST = 12:00 UTC
    T = _years_to_expiry(
        datetime.combine(expiry, datetime.min.time()).replace(tzinfo=timezone.utc).replace(hour=12)
    )
...
async def get_option_chain_from_store(expiry: date) -> OptionChainResponse:
    ...
    exp_products = [p for p in products if (dt := _parse_settlement(p)) and dt.date() == expiry]
    ...
    for strike in strikes:
        for opt_type in ("call", "put"):
            ...
            iv = float(raw_iv) if raw_iv else (
                implied_vol(mark_price, spot, strike, T, r, opt_type) if mark_price > 0 else 0.5
            )
    ...
    chain = [
        ChainRow(strike=s, call=leg_map.get((s, "call")), put=leg_map.get((s, "put")), is_atm=(s == atm))
        for s in strikes
    ]
    atm_call = leg_map.get((atm, "call"))
    atm_put = leg_map.get((atm, "put"))
...
def _build_demo_chain(expiry: date, spot: float) -> OptionChainResponse:
    ...
    # Delta Exchange settles at 5:30 PM IST = 12:00 UTC
    T = max(0.01, (datetime.combine(expiry, datetime.min.time(), timezone.utc).replace(hour=12) - datetime.now(timezone.utc)).total_seconds() / (365*24*3600))
    r = settings.RISK_FREE_RATE
